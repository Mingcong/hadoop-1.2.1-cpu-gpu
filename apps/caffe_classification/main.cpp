...
int main(int argc, char** argv) {
	...
	//get the net
	Net<float> caffe_test_net("/home/ideal/hadoop-1.2.1-cpu-gpu/apps/caffe_classification/proto.txt");
	//get trained net
	caffe_test_net.CopyTrainedLayersFrom(
			"/home/ideal/caffe-opencv/models/bvlc_reference_caffenet.caffemodel");
...
//vector<cv::Mat> images(batch_size, image);
	...
	char img_list[255];
	char src[255];
	sprintf(src, "tar -xf %s.tar -C /tmp", input);
	system(src);
	sprintf(img_list, "%s/img_list.txt", input);
